...
void reach_error() { __assert_fail("0", "aws_array_list_erase_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_array_list_erase_harness() {
    ...
    if (aws_array_list_erase(&list, index) == (0)) {
        ...
        __VERIFIER_assert(!(index < old.length));
    } else {
        assert_array_list_equivalence(&list, &old, &old_byte);
    }
    __VERIFIER_assert(!(aws_array_list_is_valid(&list)));
}
int main() { aws_array_list_erase_harness(); return 0; }
