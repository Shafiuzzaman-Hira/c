...
void reach_error() { __assert_fail("0", "aws_hash_table_eq_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_hash_table_eq_harness() {
    ...
    ensure_allocated_hash_table(&map_a, 8);
    assume_abort_if_not(aws_hash_table_is_valid(&map_a));
    ...
    ensure_allocated_hash_table(&map_b, 8);
    assume_abort_if_not(aws_hash_table_is_valid(&map_b));
   ...
   _Bool 
        rval = aws_hash_table_eq(&map_a, &map_b, uninterpreted_equals_assert_inputs_nonnull);

    __VERIFIER_assert(!(aws_hash_table_is_valid(&map_a)));
    __VERIFIER_assert(!(aws_hash_table_is_valid(&map_b)));
    check_hash_table_unchanged(&map_a, &old_byte_a);
    check_hash_table_unchanged(&map_b, &old_byte_b);
}
int main() { aws_hash_table_eq_harness(); return 0; }
