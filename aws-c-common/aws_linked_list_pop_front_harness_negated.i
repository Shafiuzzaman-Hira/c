...
void reach_error() { __assert_fail("0", "aws_linked_list_pop_front_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_linked_list_pop_front_harness() {
    ...
    __VERIFIER_assert(!(aws_linked_list_is_valid(&list)));
    __VERIFIER_assert(ret->next == 
   ((void *)0) 
   && ret->prev == 
   ((void *)0)
   );
    __VERIFIER_assert(!(list.head.next == old_next_first));
}
int main() { aws_linked_list_pop_front_harness(); return 0; }
