...
void reach_error() { __assert_fail("0", "aws_array_list_set_at_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_array_list_set_at_harness() {
   ...
   ((void *)0)
    ...
    if (!aws_array_list_set_at(&list, val, index)) {
        if (index > old.length)
            __VERIFIER_assert(!(list.length == index + 1));
    } else {
...
int main() { aws_array_list_set_at_harness(); return 0; }
