...
void reach_error() { __assert_fail("0", "aws_array_list_copy_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_array_list_copy_harness() {
   ...
   ((void *)0)
    ...
    if (!aws_array_list_copy(&from, &to)) {
        ...
        __VERIFIER_assert(!(to.length == from.length));
        __VERIFIER_assert(!(to.current_size >= (from.length * from.item_size)));
    }


    __VERIFIER_assert(!(aws_array_list_is_valid(&from)));
    __VERIFIER_assert(!(aws_array_list_is_valid(&to)));
    __VERIFIER_assert(!(from.item_size == to.item_size));
}
int main() { aws_array_list_copy_harness(); return 0; }
