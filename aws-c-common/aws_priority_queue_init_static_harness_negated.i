...
void reach_error() { __assert_fail("0", "aws_priority_queue_init_static_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_priority_queue_init_static_harness() {
    ...
    __VERIFIER_assert(!(aws_priority_queue_is_valid(&queue)));
    __VERIFIER_assert(queue.container.alloc == 
   ((void *)0)
   );
    __VERIFIER_assert(!(queue.container.item_size == item_size));
    __VERIFIER_assert(!(queue.container.length == 0));
    __VERIFIER_assert(!(queue.container.current_size == initial_item_allocation * item_size));
    assert_bytes_match((uint8_t *)queue.container.data, raw_array, len);
}
int main() { aws_priority_queue_init_static_harness(); return 0; }
