...
void reach_error() { __assert_fail("0", "aws_hash_table_get_entry_count_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_hash_table_get_entry_count_harness() {
    ...
    ensure_allocated_hash_table(&table, 
                                       (18446744073709551615UL)
                                               );
    assume_abort_if_not(aws_hash_table_is_valid(&table));
    ...
    __VERIFIER_assert(!(rval == old_entry_count));

    __VERIFIER_assert(!(aws_hash_table_is_valid(&table)));
