...
void reach_error() { __assert_fail("0", "aws_linked_list_push_front_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_linked_list_push_front_harness() {
    ...
    __VERIFIER_assert(!(aws_linked_list_node_prev_is_valid(&to_add)));
    __VERIFIER_assert(!(aws_linked_list_node_next_is_valid(&to_add)));
    __VERIFIER_assert(!(list.head.next == &to_add));
    __VERIFIER_assert(!(to_add.next == old_first));
}
int main() { aws_linked_list_push_front_harness(); return 0; }
