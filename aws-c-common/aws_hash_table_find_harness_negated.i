...
void reach_error() { __assert_fail("0", "aws_hash_table_find_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_hash_table_find_harness() {
    ...
    __VERIFIER_assert(!(rval == (0)));
    if (p_elem) {
        __VERIFIER_assert(!(((((sizeof(*(p_elem)))) == 0) || (((p_elem))))));
        __VERIFIER_assert(!(p_elem->key == key || uninterpreted_equals(p_elem->key, key)));
    }
    __VERIFIER_assert(!(aws_hash_table_is_valid(&map)));
    check_hash_table_unchanged(&map, &old_byte);
}
int main() { aws_hash_table_find_harness(); return 0; }
