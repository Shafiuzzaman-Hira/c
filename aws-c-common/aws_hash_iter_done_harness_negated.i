...
void reach_error() { __assert_fail("0", "aws_hash_iter_done_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_hash_iter_done_harness() {
    ...
    struct aws_hash_table map;
    ensure_allocated_hash_table(&map, 
                                     (18446744073709551615UL)
                                                   );
    assume_abort_if_not(aws_hash_table_is_valid(&map));
   ...
   _Bool 
        rval = aws_hash_iter_done(&iter);

    __VERIFIER_assert(!(aws_hash_iter_is_valid(&iter)));
    __VERIFIER_assert(!(rval == (iter.status == AWS_HASH_ITER_STATUS_DONE)));
    __VERIFIER_assert(!(iter.status == old_status));
    __VERIFIER_assert(!(aws_hash_table_is_valid(&map)));
    check_hash_table_unchanged(&map, &old_byte);
}
int main() { aws_hash_iter_done_harness(); return 0; }
