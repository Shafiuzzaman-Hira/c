...
void reach_error() { __assert_fail("0", "aws_linked_list_remove_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_linked_list_remove_harness() {
    ...
    __VERIFIER_assert(!(aws_linked_list_node_next_is_valid(&prev)));
    __VERIFIER_assert(!(aws_linked_list_node_prev_is_valid(&next)));

    __VERIFIER_assert(!(prev.next == &next));
    __VERIFIER_assert(node.next == 
   ((void *)0)
   );
    __VERIFIER_assert(node.prev == 
   ((void *)0)
   );
}
int main() { aws_linked_list_remove_harness(); return 0; }
