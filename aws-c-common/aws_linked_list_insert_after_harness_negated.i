...
void reach_error() { __assert_fail("0", "aws_linked_list_insert_after_harness.i", 208, "reach_error"); }
extern void abort(void);
...
void aws_linked_list_insert_after_harness() {
    ...
    __VERIFIER_assert(!(aws_linked_list_node_prev_is_valid(&to_add)));
    __VERIFIER_assert(!(aws_linked_list_node_next_is_valid(&to_add)));
    __VERIFIER_assert(!(aws_linked_list_node_prev_is_valid(&after_next)));

    __VERIFIER_assert(!(after.next == &to_add));
    __VERIFIER_assert(!(after_next.prev == &to_add));
}
int main() { aws_linked_list_insert_after_harness(); return 0; }
