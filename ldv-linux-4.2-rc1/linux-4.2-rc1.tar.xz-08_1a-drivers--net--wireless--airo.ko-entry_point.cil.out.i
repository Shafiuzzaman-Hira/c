...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "linux-4.2-rc1.tar.xz-08_1a-drivers--net--wireless--airo.ko-entry_point.cil.out.c", 3, __extension__ __PRETTY_FUNCTION__); })); }
...
typedef enum ldv_25491 socket_state;
struct socket_wq {
   wait_queue_head_t wait ;
...
typedef enum ldv_28305 phy_interface_t;
enum ldv_28359 {
    MDIOBUS_ALLOCATED = 1,
...
typedef enum ldv_35033 mic_error;
struct proc_data {
   int release_buffer ;
   int readlen ;
   char *rbuffer ;
...
__inline static int ldv_request_irq_5(unsigned int irq___0 , irqreturn_t (*handler)(int ,
                                                                                    void * ) ,
                                      unsigned long flags , char const *name , void *dev ) ;
extern void free_irq(unsigned int , void * ) ;
void ldv_free_irq_6(unsigned int ldv_func_arg1 , void *ldv_func_arg2 ) ;
...
void ldv_free_netdev_10(struct net_device *dev ) ;
...
static int airo_open(struct net_device *dev )
{
  ...
  if ((unsigned long )ai->wifidev != (unsigned long )dev) {
    ...
    rc = ldv_request_irq_5((unsigned int )dev->irq, & airo_interrupt, 128UL, (char const *)(& dev->name),
                           (void *)dev);
    if (rc != 0) {
...
static int airo_close(struct net_device *dev )
{
  ...
  if ((unsigned long )ai->wifidev != (unsigned long )dev) {
    ...
    ldv_free_irq_6((unsigned int )dev->irq, (void *)dev);
    set_bit(0L, (unsigned long volatile *)(& ai->jobs));
    kthread_stop(ai->airo_thread_task);
  } else {
  }
...
void stop_airo_card(struct net_device *dev , int freeres )
{
  ...
  if (tmp != 0) {
    ldv_unregister_netdev_7(dev);
    if ((unsigned long )ai->wifidev != (unsigned long )((struct net_device *)0)) {
      ldv_unregister_netdev_8(ai->wifidev);
      ldv_free_netdev_9(ai->wifidev);
...
static struct net_device *init_wifidev(struct airo_info *ai , struct net_device *ethdev )
{
  ...
  err = ldv_register_netdev_11(dev);
  if (err < 0) {
                                          ...
                                          struct pci_dev *pci , struct device *dmdev )
{
  ...
  return (dev);
  err_out_wifi:
  ldv_unregister_netdev_14(ai->wifidev);
  ldv_free_netdev_15(ai->wifidev);
  err_out_reg:
  ldv_unregister_netdev_16(dev);
  err_out_map:
                       ...
                       int perm , int lock )
{
  ...
  if (tmp != 0L) {
    warn_slowpath_null("/work/ldvuser/mutilin/launch/work/current--X--drivers/--X--defaultlinux-4.2-rc1.tar.xz--X--08_1a--X--cpachecker/linux-4.2-rc1.tar.xz/csd_deg_dscv/12538/dscv_tempdir/dscv/ri/08_1a/drivers/net/wireless/airo.c",
                       5209);
  } else {
  }
...
static int airo_init_module(void)
{
  ...
  i = ldv___pci_register_driver_18(& airo_driver, & __this_module, "airo");
...
static void airo_cleanup_module(void)
{
  ...
  ldv_pci_unregister_driver_19(& airo_driver);
  remove_proc_entry("driver/aironet", (struct proc_dir_entry *)0);
  return;
}
}
