// This file is part of the SV-Benchmarks collection of verification tasks:
// https://gitlab.com/sosy-lab/benchmarking/sv-benchmarks
//
// SPDX-FileCopyrightText: 2022 Jana (Philipp) Berger
//
// SPDX-License-Identifier: GPL-3.0-or-later

// Prototype declarations of the functions used to communicate with the model checkers
extern unsigned long __VERIFIER_nondet_ulong();
extern long __VERIFIER_nondet_long();
extern unsigned char __VERIFIER_nondet_uchar();
extern char __VERIFIER_nondet_char();
extern unsigned short __VERIFIER_nondet_ushort();
extern short __VERIFIER_nondet_short();
extern float __VERIFIER_nondet_float();
extern double __VERIFIER_nondet_double();

extern void abort(void);
extern void __assert_fail(const char *, const char *, unsigned int, const char *) __attribute__ ((__nothrow__ , __leaf__)) __attribute__ ((__noreturn__));
void reach_error() { __assert_fail("0", "Req1_Prop1_Batch10PS_CN_500.c", 13, "reach_error"); }
void __VERIFIER_assert(int cond) { if(!(cond)) { ERROR: {reach_error();abort();} } return; }
void assume_abort_if_not(int cond) { if(!cond) { abort(); } }



#define max(a,b) (((a) > (b)) ? (a) : (b))
#define min(a,b) (((a) < (b)) ? (a) : (b))
#define abs(a) (((a) < 0 ) ? -(a) : (a))





// Function prototypes


// Internal control logic variables
unsigned char isInitial = 0;

// Signal variables
unsigned short int var_1_1 = 4;
unsigned short int var_1_4 = 58792;
unsigned short int var_1_5 = 8;
unsigned short int var_1_6 = 64;
unsigned short int var_1_7 = 16;
unsigned char var_1_8 = 0;
unsigned char var_1_9 = 1;
signed short int var_1_10 = 1000;
signed short int var_1_11 = 1;
signed short int var_1_12 = -25;
signed short int var_1_13 = 25;
unsigned short int var_1_14 = 5;
unsigned short int var_1_16 = 20511;
unsigned long int var_1_17 = 50;
unsigned short int var_1_18 = 10;
unsigned short int var_1_19 = 2;
float var_1_20 = 100000.5;
float var_1_21 = 31.9;
float var_1_22 = -0.6;
unsigned short int var_1_23 = 32;
float var_1_24 = 24.5;
unsigned short int var_1_25 = 41226;
unsigned short int var_1_26 = 46406;
unsigned short int var_1_27 = 32;
signed long int var_1_28 = -128;
unsigned char var_1_29 = 0;
signed long int var_1_30 = -128;
double var_1_31 = -0.8;
double var_1_32 = 49.9;
double var_1_33 = 8.375;
unsigned char var_1_34 = 10;
unsigned char var_1_35 = 1;
unsigned char var_1_36 = 1;
unsigned char var_1_37 = 0;
signed long int var_1_38 = -100;
unsigned short int var_1_39 = 32;
unsigned char var_1_40 = 128;
unsigned char var_1_41 = 128;
unsigned char var_1_42 = 5;
unsigned char var_1_43 = 64;
unsigned char var_1_44 = 16;
unsigned char var_1_45 = 1;
signed char var_1_46 = -1;
double var_1_47 = 4.2;
double var_1_48 = 64.8;
signed char var_1_49 = 2;
signed char var_1_50 = 50;
unsigned short int var_1_51 = 10;
unsigned short int var_1_52 = 100;
double var_1_53 = 16.5;
float var_1_54 = 7.6;
double var_1_55 = 2.5;
unsigned char var_1_56 = 1;
unsigned char var_1_57 = 0;
unsigned char var_1_58 = 0;
unsigned char var_1_59 = 50;
unsigned short int var_1_60 = 5;
float var_1_61 = 64.9;
float var_1_62 = 31.8;
float var_1_63 = -0.875;
signed long int var_1_64 = 500;
signed long int var_1_65 = -25;
signed short int var_1_66 = 100;
unsigned short int var_1_67 = 25;
unsigned short int var_1_68 = 32;
double var_1_69 = 4.75;
double var_1_70 = 3.2;
double var_1_71 = 16.875;
float var_1_72 = 2.5;
float var_1_73 = 2.2;
float var_1_74 = 50.6;
float var_1_75 = 256.4;
float var_1_76 = 10.75;
float var_1_77 = 31.75;
float var_1_78 = 5.375;
signed char var_1_79 = 5;
signed char var_1_80 = -16;
unsigned long int var_1_81 = 100;
unsigned short int var_1_82 = 0;
unsigned long int var_1_83 = 2039033143;
unsigned long int var_1_84 = 32;
double var_1_85 = 32.125;
signed short int var_1_86 = -10;
signed long int var_1_87 = -100;
double var_1_88 = 999999999.5;
unsigned char var_1_89 = 10;
double var_1_90 = 0.0;
signed short int var_1_91 = -1;
unsigned char var_1_92 = 100;
unsigned char var_1_93 = 1;
unsigned char var_1_94 = 16;
unsigned char var_1_95 = 50;
unsigned char var_1_96 = 2;
signed char var_1_97 = 10;
signed char var_1_98 = 64;
signed short int var_1_99 = -25;
unsigned long int var_1_100 = 200;
signed long int var_1_101 = 100;
unsigned char var_1_102 = 1;
unsigned char var_1_103 = 4;
unsigned char var_1_104 = 1;
unsigned char var_1_105 = 4;
double var_1_106 = -0.75;
unsigned short int var_1_107 = 256;
unsigned short int var_1_108 = 100;
signed long int var_1_109 = 128;
double var_1_110 = 64.1;
float var_1_111 = 9.5;
float var_1_112 = 10.1;
unsigned long int var_1_113 = 4;
unsigned long int var_1_114 = 16;
unsigned long int var_1_115 = 10;
unsigned long int var_1_116 = 3555471535;
signed short int var_1_117 = 128;
float var_1_118 = 5.6;
unsigned char var_1_119 = 5;
unsigned char var_1_120 = 5;
unsigned char var_1_121 = 0;
unsigned char var_1_122 = 100;
float var_1_123 = 7.4;
unsigned long int var_1_124 = 500;
signed short int var_1_125 = -1;
unsigned long int var_1_126 = 0;
unsigned char var_1_127 = 0;
unsigned char var_1_128 = 1;
signed short int var_1_129 = 100;
float var_1_130 = 9.6;
unsigned char var_1_131 = 10;
double var_1_132 = 10000000.2;
signed long int var_1_133 = 256;

// Calibration values

// Last'ed variables
unsigned char last_1_var_1_8 = 0;

// Additional functions


void initially(void) {
}



void step(void) {
	// From: CodeObject1
	if (var_1_20 <= var_1_21) {
		if ((var_1_21 + var_1_20) < (abs (var_1_22))) {
			if (var_1_20 <= (min (var_1_22 , var_1_21))) {
				var_1_19 = var_1_23;
			} else {
				if (var_1_20 != var_1_24) {
					var_1_19 = ((max ((abs (var_1_25)) , (abs (var_1_26)))) - var_1_27);
				} else {
					var_1_19 = (min (var_1_26 , (abs (var_1_23))));
				}
			}
		}
	}


	// From: CodeObject2
	if (var_1_19 < (~ var_1_26)) {
		if (var_1_29) {
			var_1_28 = (abs (var_1_23));
		}
	} else {
		var_1_28 = (min (var_1_30 , var_1_26));
	}


	// From: CodeObject3
	var_1_31 = (min (var_1_32 , var_1_33));


	// From: CodeObject4
	if (var_1_19 > var_1_28) {
		if ((var_1_30 * var_1_27) <= var_1_23) {
			var_1_34 = 2;
		} else {
			var_1_34 = (abs (var_1_35));
		}
	}


	// From: CodeObject5
	if (var_1_20 < var_1_21) {
		var_1_36 = var_1_37;
	}


	// From: CodeObject6
	if ((min (var_1_22 , var_1_32)) >= var_1_20) {
		var_1_38 = -64;
	}


	// From: CodeObject7
	if (var_1_27 > (abs (var_1_25))) {
		var_1_39 = (var_1_35 + (min (500 , var_1_27)));
	}


	// From: CodeObject8
	if (var_1_26 < var_1_25) {
		if (var_1_36 || (var_1_20 >= (abs (var_1_21)))) {
			if ((32 >= var_1_26) && (var_1_38 >= (var_1_25 | var_1_19))) {
				if ((var_1_19 * (min (var_1_23 , var_1_26))) >= var_1_30) {
					var_1_40 = (var_1_41 - (max (var_1_42 , (var_1_43 - var_1_44))));
				} else {
					if (var_1_21 <= -0.6f) {
						var_1_40 = (max (var_1_42 , (min ((var_1_43 + var_1_44) , var_1_35))));
					} else {
						var_1_40 = (var_1_41 - var_1_44);
					}
				}
			} else {
				if (var_1_31 > var_1_21) {
					var_1_40 = (max (var_1_41 , (abs (var_1_43))));
				}
			}
		}
	}


	// From: CodeObject9
	if (var_1_29) {
		var_1_45 = (! var_1_37);
	}


	// From: CodeObject10
	if (var_1_44 == var_1_23) {
		if ((abs (var_1_33)) < (var_1_47 - var_1_48)) {
			if (-256 >= var_1_43) {
				var_1_46 = (var_1_44 - var_1_49);
			} else {
				var_1_46 = (var_1_44 - (max (var_1_49 , var_1_50)));
			}
		} else {
			if (var_1_39 <= var_1_49) {
				var_1_46 = (var_1_50 - var_1_49);
			}
		}
	} else {
		var_1_46 = var_1_50;
	}


	// From: CodeObject11
	if (var_1_25 <= var_1_23) {
		var_1_51 = (abs (var_1_41));
	} else {
		if ((abs (var_1_46 + var_1_50)) > (var_1_25 + var_1_49)) {
			var_1_51 = (max (var_1_43 , var_1_41));
		} else {
			if (var_1_36) {
				var_1_51 = (var_1_41 + var_1_49);
			} else {
				var_1_51 = (abs (var_1_25));
			}
		}
	}


	// From: CodeObject12
	if ((- var_1_39) >= (var_1_41 * var_1_40)) {
		var_1_52 = (min ((abs (var_1_35)) , var_1_27));
	}


	// From: CodeObject13
	if ((var_1_41 / var_1_25) <= (max ((~ var_1_30) , var_1_43))) {
		if (((~ var_1_40) <= var_1_34) && ((var_1_24 + var_1_54) >= var_1_21)) {
			var_1_53 = (min (var_1_32 , (abs (var_1_33))));
		}
	} else {
		if ((2 | var_1_44) >= var_1_43) {
			var_1_53 = (abs (var_1_32));
		} else {
			var_1_53 = (min (var_1_32 , (max (var_1_33 , var_1_55))));
		}
	}


	// From: CodeObject14
	if (((var_1_49 - 4) | 8) > 16) {
		var_1_56 = (var_1_57 || var_1_58);
	} else {
		var_1_56 = (var_1_36 && var_1_58);
	}


	// From: CodeObject15
	if (var_1_46 < (min (var_1_52 , (max (var_1_23 , var_1_30))))) {
		if ((var_1_40 >= var_1_43) || var_1_58) {
			var_1_59 = (abs (abs (abs (var_1_41))));
		}
	}


	// From: CodeObject16
	if ((var_1_49 >> var_1_41) >= (min (var_1_25 , (var_1_35 * var_1_30)))) {
		var_1_60 = ((min (var_1_26 , var_1_25)) - var_1_42);
	}


	// From: CodeObject17
	if ((abs (15.9 + var_1_33)) > (min (var_1_20 , var_1_48))) {
		var_1_61 = (var_1_62 + var_1_63);
	}


	// From: CodeObject18
	if (var_1_58 || var_1_36) {
		if (var_1_31 < var_1_24) {
			var_1_64 = var_1_41;
		}
	}


	// From: CodeObject19
	if (! (var_1_53 > var_1_24)) {
		if (15.8f >= var_1_20) {
			var_1_65 = var_1_44;
		}
	} else {
		var_1_65 = (max ((abs (var_1_60 - var_1_51)) , (var_1_49 + var_1_23)));
	}


	// From: CodeObject20
	if ((var_1_41 - 50) >= var_1_50) {
		var_1_66 = 2;
	}


	// From: CodeObject21
	var_1_67 = (var_1_25 - var_1_27);


	// From: CodeObject22
	if (var_1_57) {
		var_1_68 = (abs (var_1_49));
	}


	// From: CodeObject23
	if (var_1_44 > 64) {
		if (var_1_50 > ((abs (var_1_44)) & var_1_60)) {
			var_1_69 = ((min (var_1_70 , (abs (var_1_62)))) - (max (999.6 , var_1_71)));
		}
	} else {
		var_1_69 = (var_1_71 - var_1_70);
	}


	// From: CodeObject24
	if (var_1_66 >= var_1_19) {
		if (5u <= (var_1_28 % (5u + var_1_26))) {
			var_1_72 = ((max ((var_1_73 - var_1_74) , (var_1_75 - var_1_76))) + (var_1_77 + var_1_78));
		}
	}


	// From: CodeObject25
	if (var_1_29) {
		var_1_79 = ((abs (var_1_44)) + var_1_80);
	}


	// From: CodeObject26
	if (var_1_65 < (min (var_1_40 , (-16 + var_1_66)))) {
		if (var_1_51 > var_1_82) {
			if ((min (var_1_38 , (var_1_41 ^ var_1_23))) < var_1_43) {
				var_1_81 = (max ((max ((max (var_1_82 , var_1_19)) , var_1_42)) , (10u + (var_1_83 - var_1_50))));
			} else {
				if (((min (var_1_70 , var_1_75)) * var_1_72) < var_1_63) {
					var_1_81 = (abs (max (10000u , var_1_68)));
				} else {
					var_1_81 = (max (var_1_25 , var_1_23));
				}
			}
		}
	}


	// From: CodeObject27
	if (var_1_82 == var_1_43) {
		var_1_84 = (abs (var_1_26));
	}


	// From: CodeObject28
	if (var_1_23 == 4) {
		if (var_1_32 != var_1_63) {
			var_1_85 = (max (var_1_33 , 256.5));
		}
	}


	// From: CodeObject29
	if (var_1_56) {
		var_1_86 = (abs (min (var_1_42 , var_1_79)));
	}


	// From: CodeObject30
	if (var_1_21 > ((- var_1_22) + (abs (var_1_74)))) {
		if ((min (var_1_54 , var_1_53)) < var_1_61) {
			var_1_87 = (max (-1000000000 , var_1_42));
		}
	}


	// From: CodeObject31
	if (var_1_82 > (var_1_49 / (max (var_1_41 , var_1_43)))) {
		var_1_88 = (abs (abs (var_1_76 + var_1_77)));
	} else {
		if (var_1_62 <= (max (var_1_76 , var_1_33))) {
			if (var_1_45) {
				if (var_1_58) {
					var_1_88 = ((abs (var_1_77)) + var_1_73);
				}
			}
		}
	}


	// From: CodeObject32
	if (var_1_45) {
		if (var_1_37) {
			var_1_89 = (min (var_1_50 , var_1_42));
		}
	}


	// From: CodeObject33
	if (var_1_68 < var_1_25) {
		if (var_1_35 >= 8) {
			if (var_1_55 >= var_1_73) {
				if (var_1_40 >= var_1_81) {
					var_1_90 = (max (var_1_71 , var_1_73));
				}
			}
		} else {
			var_1_90 = (min (16.75 , (min (9999.2 , 10.75))));
		}
	} else {
		var_1_90 = var_1_71;
	}


	// From: CodeObject34
	if (var_1_35 >= (var_1_27 / (var_1_41 - var_1_92))) {
		if ((var_1_44 > (var_1_51 / 32)) && var_1_93) {
			var_1_91 = (max (var_1_50 , var_1_40));
		}
	}


	// From: CodeObject35
	if (var_1_53 <= (abs (var_1_33 / 127.4))) {
		if ((abs (abs (var_1_30))) >= var_1_84) {
			var_1_94 = ((var_1_43 - (min (var_1_44 , var_1_95))) + (min ((4 + var_1_96) , var_1_50)));
		} else {
			var_1_94 = (min (((var_1_43 - var_1_44) + var_1_95) , (min (var_1_42 , var_1_49))));
		}
	} else {
		if (var_1_36 && (var_1_54 >= 100.2f)) {
			var_1_94 = (min ((min ((abs (var_1_43)) , 16)) , var_1_42));
		}
	}


	// From: CodeObject36
	if (var_1_93) {
		var_1_97 = (abs (var_1_92 - (var_1_98 - var_1_95)));
	}


	// From: CodeObject37
	if ((var_1_85 <= var_1_24) && (! var_1_29)) {
		var_1_99 = (min ((var_1_59 + var_1_97) , -50));
	} else {
		var_1_99 = (min ((abs (var_1_89)) , (min (var_1_94 , var_1_40))));
	}


	// From: CodeObject38
	var_1_100 = (2519551515u - var_1_89);


	// From: CodeObject39
	if (var_1_73 > (max (var_1_62 , (var_1_31 + var_1_76)))) {
		var_1_101 = var_1_86;
	}


	// From: CodeObject40
	if (var_1_93) {
		var_1_102 = (! var_1_37);
	}


	// From: CodeObject41
	if (var_1_41 < var_1_42) {
		var_1_103 = var_1_42;
	}


	// From: CodeObject42
	if (var_1_66 >= var_1_82) {
		if ((var_1_35 | var_1_50) <= var_1_89) {
			var_1_104 = (var_1_36 && var_1_58);
		}
	}


	// From: CodeObject43
	if ((var_1_40 / (abs (var_1_92))) <= (abs (max (var_1_95 , var_1_23)))) {
		if (var_1_55 < var_1_106) {
			var_1_105 = var_1_35;
		}
	} else {
		var_1_105 = (var_1_49 + ((max (var_1_43 , var_1_98)) - 4));
	}


	// From: CodeObject44
	if (var_1_82 >= ((abs (var_1_108)) | var_1_19)) {
		var_1_107 = ((abs (59809)) - (var_1_35 + var_1_42));
	}


	// From: CodeObject45
	if (0u < var_1_52) {
		var_1_109 = ((abs (var_1_27)) + var_1_86);
	} else {
		if ((max (var_1_44 , var_1_38)) > var_1_66) {
			var_1_109 = (var_1_94 + var_1_43);
		}
	}


	// From: CodeObject46
	if (var_1_82 > var_1_51) {
		var_1_110 = (abs (var_1_33));
	}


	// From: CodeObject47
	if (var_1_52 < var_1_103) {
		var_1_111 = (abs (var_1_63));
	} else {
		if (var_1_57) {
			if (! var_1_93) {
				if (((var_1_87 * var_1_107) ^ var_1_89) < (var_1_34 - 256)) {
					var_1_111 = var_1_55;
				}
			} else {
				var_1_111 = (min (var_1_77 , (var_1_78 + var_1_75)));
			}
		}
	}


	// From: CodeObject48
	if (var_1_65 <= (var_1_19 >> (max (var_1_113 , var_1_114)))) {
		var_1_112 = var_1_63;
	}


	// From: CodeObject49
	if ((var_1_100 ^ (var_1_95 * var_1_68)) < (var_1_25 + (var_1_51 / var_1_26))) {
		var_1_115 = (((abs (var_1_116)) - var_1_114) - (min (var_1_40 , (min (var_1_26 , var_1_51)))));
	}


	// From: CodeObject50
	if (var_1_58) {
		var_1_117 = var_1_97;
	} else {
		var_1_117 = var_1_80;
	}


	// From: CodeObject51
	var_1_118 = var_1_70;


	// From: CodeObject52
	if (var_1_57) {
		var_1_119 = var_1_50;
	}


	// From: CodeObject53
	if (var_1_121) {
		var_1_120 = 50;
	}


	// From: CodeObject54
	var_1_122 = var_1_92;


	// From: CodeObject55
	if (var_1_57) {
		var_1_123 = 4.4f;
	} else {
		var_1_123 = var_1_63;
	}


	// From: CodeObject56
	if (var_1_104) {
		var_1_124 = var_1_108;
	} else {
		var_1_124 = var_1_103;
	}


	// From: CodeObject57
	if (var_1_93) {
		var_1_125 = var_1_120;
	} else {
		var_1_125 = var_1_103;
	}


	// From: CodeObject58
	if (var_1_45) {
		var_1_126 = var_1_51;
	}


	// From: CodeObject59
	if (var_1_57) {
		var_1_127 = var_1_37;
	} else {
		var_1_127 = var_1_58;
	}


	// From: CodeObject60
	if (var_1_45) {
		var_1_128 = 0;
	}


	// From: CodeObject61
	if (var_1_93) {
		var_1_129 = var_1_43;
	}


	// From: CodeObject62
	var_1_130 = var_1_33;


	// From: CodeObject63
	if (var_1_57) {
		var_1_131 = var_1_98;
	} else {
		var_1_131 = var_1_41;
	}


	// From: CodeObject64
	if (var_1_128) {
		var_1_132 = (256.5 + (256.5 + var_1_78));
	}


	// From: CodeObject65
	if (var_1_105 >= (min (var_1_94 , var_1_115))) {
		if (var_1_110 <= var_1_90) {
			var_1_133 = -1;
		}
	}


	// From: Req1Batch10PS_CN_500
	unsigned short int stepLocal_1 = var_1_6;
	unsigned char stepLocal_0 = last_1_var_1_8;
	if (last_1_var_1_8 && stepLocal_0) {
		var_1_1 = (((min (56892 , var_1_4)) - var_1_5) - (max (var_1_6 , var_1_7)));
	} else {
		if ((max (var_1_7 , var_1_5)) < stepLocal_1) {
			var_1_1 = var_1_5;
		}
	}


	// From: Req2Batch10PS_CN_500
	var_1_8 = (((var_1_7 >= var_1_5) && (var_1_4 >= var_1_1)) || var_1_9);


	// From: Req4Batch10PS_CN_500
	if (var_1_8) {
		var_1_14 = ((abs (var_1_4)) - (max (var_1_5 , (var_1_16 - var_1_11))));
	}


	// From: Req5Batch10PS_CN_500
	signed long int stepLocal_4 = max (var_1_13 , (var_1_16 << var_1_7));
	if (stepLocal_4 >= var_1_14) {
		var_1_17 = var_1_11;
	} else {
		var_1_17 = (var_1_6 + var_1_5);
	}


	// From: Req6Batch10PS_CN_500
	unsigned short int stepLocal_5 = var_1_5;
	if (stepLocal_5 == var_1_16) {
		if (var_1_8) {
			var_1_18 = (abs ((var_1_16 + 30393) - var_1_5));
		} else {
			var_1_18 = var_1_4;
		}
	} else {
		if (var_1_9) {
			var_1_18 = 200;
		}
	}


	// From: Req3Batch10PS_CN_500
	unsigned char stepLocal_3 = var_1_9;
	unsigned long int stepLocal_2 = var_1_17;
	if (stepLocal_2 < var_1_6) {
		if ((var_1_6 != var_1_5) || stepLocal_3) {
			var_1_10 = (((10000 - var_1_11) - var_1_5) + (abs (var_1_12)));
		} else {
			var_1_10 = var_1_4;
		}
	} else {
		var_1_10 = var_1_13;
	}
}



void updateVariables() {
	var_1_4 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_4 >= 49150);
	assume_abort_if_not(var_1_4 <= 65534);
	var_1_5 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_5 >= 0);
	assume_abort_if_not(var_1_5 <= 16383);
	var_1_6 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_6 >= 0);
	assume_abort_if_not(var_1_6 <= 32767);
	var_1_7 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_7 >= 0);
	assume_abort_if_not(var_1_7 <= 32767);
	var_1_9 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_9 >= 1);
	assume_abort_if_not(var_1_9 <= 1);
	var_1_11 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_11 >= 0);
	assume_abort_if_not(var_1_11 <= 8191);
	var_1_12 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_12 >= -16383);
	assume_abort_if_not(var_1_12 <= 16383);
	var_1_13 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_13 >= -32767);
	assume_abort_if_not(var_1_13 <= 32766);
	var_1_16 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_16 >= 16383);
	assume_abort_if_not(var_1_16 <= 32767);
	var_1_20 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_20 >= -922337.2036854776000e+13F && var_1_20 <= -1.0e-20F) || (var_1_20 <= 9223372.036854776000e+12F && var_1_20 >= 1.0e-20F ));
	var_1_21 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_21 >= -922337.2036854776000e+13F && var_1_21 <= -1.0e-20F) || (var_1_21 <= 9223372.036854776000e+12F && var_1_21 >= 1.0e-20F ));
	var_1_22 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_22 >= -922337.2036854776000e+13F && var_1_22 <= -1.0e-20F) || (var_1_22 <= 9223372.036854776000e+12F && var_1_22 >= 1.0e-20F ));
	var_1_23 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_23 >= 0);
	assume_abort_if_not(var_1_23 <= 65534);
	var_1_24 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_24 >= -922337.2036854776000e+13F && var_1_24 <= -1.0e-20F) || (var_1_24 <= 9223372.036854776000e+12F && var_1_24 >= 1.0e-20F ));
	var_1_25 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_25 >= 32767);
	assume_abort_if_not(var_1_25 <= 65534);
	var_1_26 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_26 >= 32767);
	assume_abort_if_not(var_1_26 <= 65534);
	var_1_27 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_27 >= 0);
	assume_abort_if_not(var_1_27 <= 32767);
	var_1_29 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_29 >= 0);
	assume_abort_if_not(var_1_29 <= 1);
	var_1_30 = __VERIFIER_nondet_long();
	assume_abort_if_not(var_1_30 >= -2147483647);
	assume_abort_if_not(var_1_30 <= 2147483646);
	var_1_32 = __VERIFIER_nondet_double();
	assume_abort_if_not((var_1_32 >= -922337.2036854765600e+13F && var_1_32 <= -1.0e-20F) || (var_1_32 <= 9223372.036854765600e+12F && var_1_32 >= 1.0e-20F ));
	var_1_33 = __VERIFIER_nondet_double();
	assume_abort_if_not((var_1_33 >= -922337.2036854765600e+13F && var_1_33 <= -1.0e-20F) || (var_1_33 <= 9223372.036854765600e+12F && var_1_33 >= 1.0e-20F ));
	var_1_35 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_35 >= 0);
	assume_abort_if_not(var_1_35 <= 254);
	var_1_37 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_37 >= 1);
	assume_abort_if_not(var_1_37 <= 1);
	var_1_41 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_41 >= 127);
	assume_abort_if_not(var_1_41 <= 254);
	var_1_42 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_42 >= 0);
	assume_abort_if_not(var_1_42 <= 127);
	var_1_43 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_43 >= 63);
	assume_abort_if_not(var_1_43 <= 127);
	var_1_44 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_44 >= 0);
	assume_abort_if_not(var_1_44 <= 63);
	var_1_47 = __VERIFIER_nondet_double();
	assume_abort_if_not((var_1_47 >= 0.0F && var_1_47 <= -1.0e-20F) || (var_1_47 <= 9223372.036854776000e+12F && var_1_47 >= 1.0e-20F ));
	var_1_48 = __VERIFIER_nondet_double();
	assume_abort_if_not((var_1_48 >= 0.0F && var_1_48 <= -1.0e-20F) || (var_1_48 <= 9223372.036854776000e+12F && var_1_48 >= 1.0e-20F ));
	var_1_49 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_49 >= 0);
	assume_abort_if_not(var_1_49 <= 126);
	var_1_50 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_50 >= 0);
	assume_abort_if_not(var_1_50 <= 126);
	var_1_54 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_54 >= -922337.2036854776000e+13F && var_1_54 <= -1.0e-20F) || (var_1_54 <= 9223372.036854776000e+12F && var_1_54 >= 1.0e-20F ));
	var_1_55 = __VERIFIER_nondet_double();
	assume_abort_if_not((var_1_55 >= -922337.2036854765600e+13F && var_1_55 <= -1.0e-20F) || (var_1_55 <= 9223372.036854765600e+12F && var_1_55 >= 1.0e-20F ));
	var_1_57 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_57 >= 0);
	assume_abort_if_not(var_1_57 <= 0);
	var_1_58 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_58 >= 0);
	assume_abort_if_not(var_1_58 <= 0);
	var_1_62 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_62 >= -461168.6018427382800e+13F && var_1_62 <= -1.0e-20F) || (var_1_62 <= 4611686.018427382800e+12F && var_1_62 >= 1.0e-20F ));
	var_1_63 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_63 >= -461168.6018427382800e+13F && var_1_63 <= -1.0e-20F) || (var_1_63 <= 4611686.018427382800e+12F && var_1_63 >= 1.0e-20F ));
	var_1_70 = __VERIFIER_nondet_double();
	assume_abort_if_not((var_1_70 >= 0.0F && var_1_70 <= -1.0e-20F) || (var_1_70 <= 9223372.036854765600e+12F && var_1_70 >= 1.0e-20F ));
	var_1_71 = __VERIFIER_nondet_double();
	assume_abort_if_not((var_1_71 >= 0.0F && var_1_71 <= -1.0e-20F) || (var_1_71 <= 9223372.036854765600e+12F && var_1_71 >= 1.0e-20F ));
	var_1_73 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_73 >= 0.0F && var_1_73 <= -1.0e-20F) || (var_1_73 <= 4611686.018427382800e+12F && var_1_73 >= 1.0e-20F ));
	var_1_74 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_74 >= 0.0F && var_1_74 <= -1.0e-20F) || (var_1_74 <= 4611686.018427382800e+12F && var_1_74 >= 1.0e-20F ));
	var_1_75 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_75 >= 0.0F && var_1_75 <= -1.0e-20F) || (var_1_75 <= 4611686.018427382800e+12F && var_1_75 >= 1.0e-20F ));
	var_1_76 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_76 >= 0.0F && var_1_76 <= -1.0e-20F) || (var_1_76 <= 4611686.018427382800e+12F && var_1_76 >= 1.0e-20F ));
	var_1_77 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_77 >= -230584.3009213691390e+13F && var_1_77 <= -1.0e-20F) || (var_1_77 <= 2305843.009213691390e+12F && var_1_77 >= 1.0e-20F ));
	var_1_78 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_78 >= -230584.3009213691390e+13F && var_1_78 <= -1.0e-20F) || (var_1_78 <= 2305843.009213691390e+12F && var_1_78 >= 1.0e-20F ));
	var_1_80 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_80 >= -63);
	assume_abort_if_not(var_1_80 <= 63);
	var_1_82 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_82 >= 0);
	assume_abort_if_not(var_1_82 <= 65535);
	var_1_83 = __VERIFIER_nondet_ulong();
	assume_abort_if_not(var_1_83 >= 1073741823);
	assume_abort_if_not(var_1_83 <= 2147483647);
	var_1_92 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_92 >= 1);
	assume_abort_if_not(var_1_92 <= 126);
	assume_abort_if_not(var_1_92 != 127);
	var_1_93 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_93 >= 0);
	assume_abort_if_not(var_1_93 <= 1);
	var_1_95 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_95 >= 0);
	assume_abort_if_not(var_1_95 <= 63);
	var_1_96 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_96 >= 0);
	assume_abort_if_not(var_1_96 <= 63);
	var_1_98 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_98 >= 63);
	assume_abort_if_not(var_1_98 <= 126);
	var_1_106 = __VERIFIER_nondet_double();
	assume_abort_if_not((var_1_106 >= -922337.2036854776000e+13F && var_1_106 <= -1.0e-20F) || (var_1_106 <= 9223372.036854776000e+12F && var_1_106 >= 1.0e-20F ));
	var_1_108 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_108 >= 0);
	assume_abort_if_not(var_1_108 <= 65535);
	var_1_113 = __VERIFIER_nondet_ulong();
	assume_abort_if_not(var_1_113 >= 0);
	assume_abort_if_not(var_1_113 <= 17);
	var_1_114 = __VERIFIER_nondet_ulong();
	assume_abort_if_not(var_1_114 >= 0);
	assume_abort_if_not(var_1_114 <= 17);
	var_1_116 = __VERIFIER_nondet_ulong();
	assume_abort_if_not(var_1_116 >= 3221225470);
	assume_abort_if_not(var_1_116 <= 4294967294);
	var_1_121 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_121 >= 0);
	assume_abort_if_not(var_1_121 <= 1);
}



void updateLastVariables() {
	last_1_var_1_8 = var_1_8;
}

int property() {
	return ((((((last_1_var_1_8 && last_1_var_1_8) ? (var_1_1 == ((unsigned short int) (((min (56892 , var_1_4)) - var_1_5) - (max (var_1_6 , var_1_7))))) : (((max (var_1_7 , var_1_5)) < var_1_6) ? (var_1_1 == ((unsigned short int) var_1_5)) : 1)) && (var_1_8 == ((unsigned char) (((var_1_7 >= var_1_5) && (var_1_4 >= var_1_1)) || var_1_9)))) && ((var_1_17 < var_1_6) ? (((var_1_6 != var_1_5) || var_1_9) ? (var_1_10 == ((signed short int) (((10000 - var_1_11) - var_1_5) + (abs (var_1_12))))) : (var_1_10 == ((signed short int) var_1_4))) : (var_1_10 == ((signed short int) var_1_13)))) && (var_1_8 ? (var_1_14 == ((unsigned short int) ((abs (var_1_4)) - (max (var_1_5 , (var_1_16 - var_1_11)))))) : 1)) && (((max (var_1_13 , (var_1_16 << var_1_7))) >= var_1_14) ? (var_1_17 == ((unsigned long int) var_1_11)) : (var_1_17 == ((unsigned long int) (var_1_6 + var_1_5))))) && ((var_1_5 == var_1_16) ? (var_1_8 ? (var_1_18 == ((unsigned short int) (abs ((var_1_16 + 30393) - var_1_5)))) : (var_1_18 == ((unsigned short int) var_1_4))) : (var_1_9 ? (var_1_18 == ((unsigned short int) 200)) : 1))
;
}
int main(void) {
	isInitial = 1;
	initially();

	while (1) {
		updateLastVariables();

		updateVariables();
		step();
		__VERIFIER_assert(property());
		isInitial = 0;
	}

	return 0;
}
