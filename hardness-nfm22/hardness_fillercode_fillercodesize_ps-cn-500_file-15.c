// This file is part of the SV-Benchmarks collection of verification tasks:
// https://gitlab.com/sosy-lab/benchmarking/sv-benchmarks
//
// SPDX-FileCopyrightText: 2022 Jana (Philipp) Berger
//
// SPDX-License-Identifier: GPL-3.0-or-later

// Prototype declarations of the functions used to communicate with the model checkers
extern unsigned long __VERIFIER_nondet_ulong();
extern long __VERIFIER_nondet_long();
extern unsigned char __VERIFIER_nondet_uchar();
extern char __VERIFIER_nondet_char();
extern unsigned short __VERIFIER_nondet_ushort();
extern short __VERIFIER_nondet_short();
extern float __VERIFIER_nondet_float();
extern double __VERIFIER_nondet_double();

extern void abort(void);
extern void __assert_fail(const char *, const char *, unsigned int, const char *) __attribute__ ((__nothrow__ , __leaf__)) __attribute__ ((__noreturn__));
void reach_error() { __assert_fail("0", "Req1_Prop1_Batch15PS_CN_500.c", 13, "reach_error"); }
void __VERIFIER_assert(int cond) { if(!(cond)) { ERROR: {reach_error();abort();} } return; }
void assume_abort_if_not(int cond) { if(!cond) { abort(); } }



#define max(a,b) (((a) > (b)) ? (a) : (b))
#define min(a,b) (((a) < (b)) ? (a) : (b))
#define abs(a) (((a) < 0 ) ? -(a) : (a))





// Function prototypes


// Internal control logic variables
unsigned char isInitial = 0;

// Signal variables
signed short int var_1_1 = -10;
float var_1_3 = 32.4;
float var_1_4 = 49.2;
signed short int var_1_7 = 10;
signed short int var_1_8 = 64;
signed short int var_1_9 = 256;
signed char var_1_10 = -5;
signed char var_1_11 = 5;
double var_1_12 = 256.4;
float var_1_13 = 2.6;
float var_1_14 = 128.125;
double var_1_15 = 32.5;
double var_1_16 = 10000000000.25;
signed char var_1_17 = -5;
unsigned char var_1_19 = 0;
unsigned char var_1_20 = 0;
signed long int var_1_21 = 1;
signed short int var_1_22 = 17178;
signed short int var_1_23 = 5;
signed long int var_1_25 = -4;
signed short int var_1_26 = 10;
unsigned char var_1_27 = 0;
signed short int var_1_28 = -16;
signed short int var_1_29 = 32;
unsigned char var_1_30 = 1;
unsigned char var_1_31 = 1;
signed short int var_1_32 = 0;
signed short int var_1_33 = 10;
signed char var_1_34 = -128;
signed char var_1_35 = -16;
signed char var_1_36 = 0;
signed char var_1_37 = 5;
float var_1_38 = 256.6;
float var_1_39 = 255.4;
float var_1_40 = 49.2;
float var_1_41 = 31.5;
float var_1_42 = 0.5;
float var_1_43 = 16.4;
float var_1_44 = 0.0;
float var_1_45 = 31.9;
double var_1_46 = 0.6;
unsigned char var_1_47 = 1;
unsigned char var_1_48 = 0;
unsigned char var_1_49 = 1;
float var_1_50 = 64.5;
float var_1_51 = 0.0;
signed short int var_1_52 = 10;
signed short int var_1_53 = 256;
signed short int var_1_54 = 0;
unsigned char var_1_55 = 0;
unsigned char var_1_56 = 1;
unsigned char var_1_57 = 0;
signed long int var_1_58 = 5;
unsigned char var_1_59 = 200;
unsigned char var_1_60 = 4;
signed short int var_1_61 = 1000;
float var_1_62 = 32.25;
unsigned short int var_1_63 = 0;
signed long int var_1_64 = 1562968998;
signed short int var_1_65 = -8;
unsigned long int var_1_66 = 0;
signed short int var_1_67 = 25;
unsigned long int var_1_68 = 5;
signed short int var_1_69 = -2;
unsigned short int var_1_70 = 4;
unsigned short int var_1_71 = 10000;
unsigned short int var_1_72 = 1;
signed short int var_1_73 = 10;
signed short int var_1_74 = 64;
unsigned char var_1_75 = 32;
double var_1_76 = 63.25;
unsigned short int var_1_77 = 32;
signed short int var_1_78 = -5;
signed char var_1_79 = 16;
double var_1_80 = 10.5;
double var_1_81 = 31.1;
signed char var_1_82 = 64;
signed char var_1_83 = 64;
signed char var_1_84 = 50;
signed char var_1_85 = 16;
signed char var_1_86 = 50;
unsigned long int var_1_87 = 16;
unsigned long int var_1_88 = 1931751367;
unsigned char var_1_89 = 0;
unsigned char var_1_90 = 0;
signed short int var_1_91 = -4;
signed char var_1_92 = 32;
unsigned short int var_1_93 = 1;
unsigned short int var_1_94 = 49726;
double var_1_95 = 0.875;
signed char var_1_96 = 2;
unsigned char var_1_97 = 25;
float var_1_98 = 64.75;
signed short int var_1_99 = 10;
unsigned char var_1_100 = 128;
unsigned short int var_1_101 = 256;
signed long int var_1_102 = 4;
signed short int var_1_103 = 1;
unsigned long int var_1_104 = 32;
unsigned long int var_1_105 = 10;
unsigned long int var_1_106 = 1968287276;
double var_1_107 = 4.75;
float var_1_108 = 32.8;
signed short int var_1_109 = 200;
float var_1_110 = 128.4;
float var_1_111 = 1.94;
unsigned short int var_1_112 = 64;
unsigned long int var_1_113 = 4;
unsigned long int var_1_114 = 2172001036;
unsigned char var_1_115 = 1;
unsigned char var_1_116 = 1;
signed char var_1_117 = 5;
float var_1_118 = 16.5;
signed char var_1_119 = -8;
unsigned short int var_1_120 = 10;
signed long int var_1_121 = -256;
unsigned char var_1_122 = 0;
signed char var_1_123 = 16;
signed char var_1_124 = 2;
double var_1_125 = 63.5;
signed char var_1_126 = 10;
float var_1_127 = 63.5;
unsigned short int var_1_128 = 16;
unsigned short int var_1_129 = 27605;
unsigned short int var_1_130 = 21930;

// Calibration values

// Last'ed variables
signed char last_1_var_1_10 = -5;
signed long int last_1_var_1_21 = 1;
signed long int last_1_var_1_25 = -4;

// Additional functions


void initially(void) {
}



void step(void) {
	// From: CodeObject1
	if (! var_1_27) {
		if (var_1_27) {
			var_1_26 = (abs (min (var_1_28 , var_1_29)));
		} else {
			if (var_1_29 >= 25) {
				if ((0 > var_1_29) || var_1_30) {
					var_1_26 = (abs (abs (-50)));
				} else {
					if (var_1_31) {
						var_1_26 = (var_1_32 - var_1_33);
					} else {
						var_1_26 = (abs (abs (abs (var_1_28))));
					}
				}
			} else {
				var_1_26 = -16;
			}
		}
	}


	// From: CodeObject2
	if (((1 - var_1_33) <= (~ var_1_28)) || ((64 < var_1_29) || var_1_30)) {
		if (var_1_29 > (var_1_28 >> var_1_33)) {
			if (var_1_29 < (abs (256 - var_1_33))) {
				var_1_34 = (abs (abs (var_1_35)));
			} else {
				var_1_34 = (abs (var_1_35));
			}
		} else {
			var_1_34 = var_1_35;
		}
	} else {
		var_1_34 = ((abs (var_1_36)) + var_1_37);
	}


	// From: CodeObject3
	if (var_1_26 >= ((var_1_34 * -1) ^ (abs (var_1_32)))) {
		var_1_38 = (max ((abs (var_1_39)) , var_1_40));
	} else {
		var_1_38 = ((var_1_41 + (var_1_42 + var_1_43)) - (var_1_44 - (min (63.34f , var_1_45))));
	}


	// From: CodeObject4
	if (var_1_37 < var_1_32) {
		var_1_46 = ((abs (var_1_45)) - var_1_42);
	}


	// From: CodeObject5
	if ((abs (var_1_33)) < (var_1_26 * var_1_32)) {
		if (var_1_43 > var_1_39) {
			var_1_47 = ((! var_1_27) && var_1_48);
		}
	} else {
		if (var_1_27) {
			if (var_1_39 > (var_1_42 / var_1_44)) {
				var_1_47 = var_1_49;
			}
		}
	}


	// From: CodeObject6
	if (var_1_43 > ((var_1_51 - 7.2f) - var_1_45)) {
		if (var_1_30) {
			var_1_50 = (abs (var_1_44));
		}
	} else {
		var_1_50 = (max ((var_1_41 + var_1_45) , (max (var_1_43 , var_1_40))));
	}


	// From: CodeObject7
	if (var_1_51 < (64.5f + 4.5f)) {
		if (var_1_42 > (var_1_45 - (min (var_1_43 , var_1_51)))) {
			if (var_1_46 < var_1_40) {
				var_1_52 = (abs (max (var_1_35 , var_1_29)));
			} else {
				if (var_1_36 >= var_1_32) {
					if (var_1_34 > var_1_36) {
						var_1_52 = var_1_36;
					} else {
						if (var_1_49) {
							var_1_52 = (var_1_33 - (min (8 , var_1_53)));
						}
					}
				} else {
					var_1_52 = (min (var_1_37 , (max (64 , var_1_32))));
				}
			}
		}
	} else {
		if (var_1_40 == 0.4f) {
			var_1_52 = (min (var_1_36 , (min (var_1_34 , var_1_54))));
		}
	}


	// From: CodeObject8
	var_1_55 = ((var_1_49 && var_1_56) && var_1_57);


	// From: CodeObject9
	if ((var_1_59 - var_1_60) >= var_1_33) {
		if (var_1_43 <= (abs (var_1_38))) {
			var_1_58 = (min (100 , var_1_52));
		}
	}


	// From: CodeObject10
	if ((var_1_42 + (abs (var_1_41))) > var_1_50) {
		if (var_1_46 < var_1_45) {
			if (var_1_42 >= (- (min (var_1_40 , var_1_62)))) {
				var_1_61 = (var_1_33 - var_1_60);
			} else {
				var_1_61 = (min ((abs (var_1_36)) , var_1_53));
			}
		} else {
			var_1_61 = (abs (var_1_60));
		}
	}


	// From: CodeObject11
	if ((~ (var_1_60 + var_1_59)) <= ((abs (var_1_32)) - (var_1_64 - var_1_33))) {
		var_1_63 = (var_1_59 + var_1_53);
	}


	// From: CodeObject12
	if (var_1_55) {
		var_1_65 = var_1_33;
	}


	// From: CodeObject13
	if (var_1_30) {
		var_1_66 = var_1_60;
	}


	// From: CodeObject14
	if (var_1_30) {
		var_1_67 = var_1_37;
	} else {
		var_1_67 = var_1_32;
	}


	// From: CodeObject15
	var_1_68 = var_1_63;


	// From: CodeObject16
	if (var_1_63 <= var_1_59) {
		if (var_1_56) {
			if (var_1_27) {
				var_1_69 = 5;
			}
		} else {
			if (var_1_65 < (var_1_61 & -10)) {
				var_1_69 = (abs (var_1_33 - var_1_60));
			}
		}
	}


	// From: CodeObject17
	if (var_1_46 > 256.25) {
		var_1_70 = ((31266 - (var_1_71 - var_1_60)) + var_1_53);
	} else {
		var_1_70 = (abs (var_1_60));
	}


	// From: CodeObject18
	if (var_1_28 >= var_1_68) {
		if ((abs (var_1_73)) >= (abs (min (var_1_65 , var_1_67)))) {
			if (var_1_41 > var_1_62) {
				var_1_72 = var_1_60;
			}
		} else {
			var_1_72 = var_1_71;
		}
	}


	// From: CodeObject19
	if (var_1_60 >= var_1_68) {
		var_1_74 = (max ((abs (var_1_60)) , var_1_37));
	}


	// From: CodeObject20
	if (-16 >= (max (var_1_67 , var_1_33))) {
		var_1_75 = var_1_60;
	}


	// From: CodeObject21
	if (-2 <= var_1_28) {
		if (var_1_71 >= (var_1_60 / (max (var_1_59 , var_1_77)))) {
			var_1_76 = ((abs (var_1_39)) - (max (var_1_44 , var_1_43)));
		} else {
			if ((var_1_75 & var_1_70) < var_1_54) {
				var_1_76 = var_1_43;
			}
		}
	} else {
		var_1_76 = (min (var_1_44 , var_1_41));
	}


	// From: CodeObject22
	if ((~ var_1_65) >= (var_1_37 + var_1_64)) {
		if (var_1_61 > var_1_34) {
			var_1_78 = (((max (var_1_35 , -4)) + var_1_37) + var_1_36);
		}
	} else {
		if (var_1_36 >= (var_1_54 / (min (var_1_77 , var_1_79)))) {
			var_1_78 = (32 + var_1_60);
		} else {
			var_1_78 = (var_1_60 + (min (var_1_71 , var_1_59)));
		}
	}


	// From: CodeObject23
	if ((- var_1_53) <= var_1_32) {
		var_1_80 = ((var_1_42 + var_1_43) + (abs (var_1_41)));
	} else {
		var_1_80 = (abs (max (var_1_40 , var_1_39)));
	}


	// From: CodeObject24
	if ((var_1_58 ^ var_1_71) == (32 | var_1_73)) {
		if (var_1_60 < (- var_1_29)) {
			if (var_1_52 < var_1_79) {
				var_1_81 = (abs (var_1_42 - var_1_41));
			}
		} else {
			var_1_81 = var_1_45;
		}
	} else {
		var_1_81 = (abs (var_1_39));
	}


	// From: CodeObject25
	if (var_1_52 <= var_1_33) {
		var_1_82 = ((var_1_83 - var_1_84) - (min (var_1_85 , var_1_86)));
	}


	// From: CodeObject26
	if (var_1_39 < (min (var_1_76 , var_1_44))) {
		if (var_1_41 >= ((min (var_1_46 , var_1_76)) * var_1_38)) {
			if (var_1_48) {
				var_1_87 = ((var_1_64 + 1754427220u) - (var_1_88 - 16u));
			}
		}
	}


	// From: CodeObject27
	if (var_1_67 > var_1_69) {
		var_1_89 = (var_1_27 && var_1_48);
	} else {
		var_1_89 = (! (! var_1_90));
	}


	// From: CodeObject28
	var_1_91 = (var_1_84 - (abs (var_1_29)));


	// From: CodeObject29
	if (((abs (var_1_42)) / (abs (var_1_44))) < var_1_51) {
		var_1_92 = (var_1_86 - (min (var_1_83 , var_1_84)));
	}


	// From: CodeObject30
	if ((min ((var_1_79 / var_1_77) , var_1_36)) != var_1_34) {
		if (var_1_84 >= var_1_85) {
			var_1_93 = (abs (max (var_1_33 , (abs (var_1_60)))));
		}
	} else {
		if (var_1_58 == var_1_84) {
			var_1_93 = (min ((abs (abs (5))) , (var_1_94 - var_1_33)));
		}
	}


	// From: CodeObject31
	if ((var_1_55 || (var_1_78 <= var_1_71)) || (var_1_35 <= (var_1_96 - var_1_85))) {
		var_1_95 = (max (var_1_39 , var_1_45));
	} else {
		var_1_95 = (var_1_44 - (var_1_43 + var_1_45));
	}


	// From: CodeObject32
	if (var_1_44 > (var_1_51 / var_1_98)) {
		if ((var_1_62 * var_1_81) >= (8.5 + var_1_50)) {
			if (var_1_90 && var_1_89) {
				if (var_1_34 >= var_1_29) {
					var_1_97 = ((max (var_1_85 , var_1_83)) + 8);
				} else {
					var_1_97 = var_1_60;
				}
			} else {
				var_1_97 = (abs (var_1_60));
			}
		} else {
			var_1_97 = ((min (var_1_84 , var_1_60)) + (abs (16)));
		}
	} else {
		var_1_97 = ((var_1_84 + 5) + (min ((min (var_1_86 , 2)) , (abs (var_1_83)))));
	}


	// From: CodeObject33
	if (var_1_90) {
		if ((abs (25.6)) < (min (var_1_39 , (max (var_1_40 , var_1_80))))) {
			if (var_1_54 <= var_1_77) {
				var_1_99 = ((abs (var_1_83 - var_1_97)) - var_1_33);
			}
		}
	}


	// From: CodeObject34
	if (var_1_94 >= 1) {
		var_1_100 = (abs (var_1_83));
	}


	// From: CodeObject35
	var_1_101 = var_1_86;


	// From: CodeObject36
	if ((var_1_86 ^ -16) < var_1_84) {
		if ((~ var_1_93) < var_1_84) {
			if (var_1_47) {
				var_1_102 = (max (var_1_77 , var_1_36));
			}
		} else {
			var_1_102 = ((min (var_1_61 , 64)) + (var_1_101 + var_1_86));
		}
	}


	// From: CodeObject37
	if (var_1_30 || ((max (var_1_52 , var_1_99)) <= (var_1_102 ^ var_1_88))) {
		if ((var_1_94 + var_1_86) >= var_1_87) {
			var_1_103 = (max (var_1_86 , 256));
		}
	} else {
		var_1_103 = var_1_60;
	}


	// From: CodeObject38
	var_1_104 = (abs (var_1_71));


	// From: CodeObject39
	if ((max (var_1_63 , (abs (var_1_72)))) > (var_1_103 ^ var_1_71)) {
		var_1_105 = (((var_1_106 - var_1_85) + (1862716035u - var_1_100)) - ((abs (var_1_101)) + var_1_75));
	}


	// From: CodeObject40
	if (var_1_57 || var_1_56) {
		if ((- var_1_34) >= (~ var_1_67)) {
			if (var_1_49) {
				var_1_107 = (min (var_1_39 , (abs (var_1_45))));
			} else {
				var_1_107 = (abs (min (var_1_45 , var_1_40)));
			}
		}
	}


	// From: CodeObject41
	if (var_1_103 < (abs (var_1_100))) {
		if (var_1_72 > var_1_83) {
			var_1_108 = (var_1_45 - var_1_41);
		}
	}


	// From: CodeObject42
	if (var_1_31) {
		var_1_109 = (var_1_86 + ((var_1_71 - var_1_85) - var_1_97));
	}


	// From: CodeObject43
	if (var_1_49) {
		var_1_110 = var_1_43;
	} else {
		var_1_110 = (abs (max (var_1_42 , var_1_40)));
	}


	// From: CodeObject44
	if (var_1_56) {
		if (var_1_31) {
			var_1_111 = (abs (32.75f));
		} else {
			var_1_111 = var_1_45;
		}
	} else {
		if (var_1_63 != var_1_106) {
			if (var_1_77 >= var_1_112) {
				var_1_111 = (abs (31.25f));
			}
		} else {
			var_1_111 = ((abs (var_1_42)) + var_1_43);
		}
	}


	// From: CodeObject45
	if (((var_1_33 | var_1_106) <= var_1_83) && (var_1_28 <= var_1_99)) {
		var_1_113 = (var_1_114 - var_1_75);
	}


	// From: CodeObject46
	if (var_1_89) {
		if (var_1_36 < (var_1_114 | var_1_100)) {
			var_1_115 = var_1_48;
		}
	}


	// From: CodeObject47
	if (var_1_57) {
		if (var_1_115) {
			var_1_116 = var_1_56;
		}
	} else {
		var_1_116 = var_1_56;
	}


	// From: CodeObject48
	if (var_1_115) {
		var_1_117 = var_1_35;
	}


	// From: CodeObject49
	if (var_1_30) {
		var_1_118 = var_1_43;
	} else {
		var_1_118 = 10.4f;
	}


	// From: CodeObject50
	var_1_119 = var_1_84;


	// From: CodeObject51
	if (var_1_55) {
		var_1_120 = var_1_53;
	}


	// From: CodeObject52
	if (var_1_27) {
		var_1_121 = var_1_67;
	}


	// From: CodeObject53
	if (var_1_90) {
		var_1_122 = 0;
	}


	// From: CodeObject54
	if (var_1_57) {
		var_1_123 = var_1_85;
	}


	// From: CodeObject55
	if (var_1_89) {
		var_1_124 = var_1_35;
	}


	// From: CodeObject56
	if (var_1_115) {
		var_1_125 = 0.25;
	}


	// From: CodeObject57
	if (var_1_31) {
		var_1_126 = var_1_83;
	}


	// From: CodeObject58
	if (var_1_57) {
		var_1_127 = var_1_41;
	}


	// From: CodeObject59
	if (var_1_96 == var_1_29) {
		var_1_128 = (((var_1_129 - 32) + var_1_130) - (abs (var_1_60)));
	} else {
		if (var_1_48 && var_1_49) {
			var_1_128 = var_1_130;
		} else {
			var_1_128 = var_1_71;
		}
	}


	// From: Req5Batch15PS_CN_500
	if (last_1_var_1_10 < last_1_var_1_25) {
		if (var_1_11 <= last_1_var_1_25) {
			var_1_19 = var_1_20;
		}
	}


	// From: Req2Batch15PS_CN_500
	var_1_10 = var_1_11;


	// From: Req3Batch15PS_CN_500
	if (var_1_4 >= (var_1_3 * (var_1_13 + var_1_14))) {
		var_1_12 = (min ((var_1_15 - var_1_16) , -0.85));
	} else {
		var_1_12 = var_1_16;
	}


	// From: Req4Batch15PS_CN_500
	unsigned char stepLocal_1 = var_1_19;
	if (var_1_12 <= (min (var_1_12 , var_1_12))) {
		if (var_1_19 || stepLocal_1) {
			var_1_17 = var_1_11;
		} else {
			var_1_17 = var_1_11;
		}
	}


	// From: Req7Batch15PS_CN_500
	if (var_1_19) {
		var_1_25 = (var_1_17 + (min (var_1_17 , (min (var_1_8 , var_1_17)))));
	} else {
		var_1_25 = var_1_10;
	}


	// From: Req1Batch15PS_CN_500
	unsigned char stepLocal_0 = var_1_12 > var_1_12;
	if (var_1_19 || stepLocal_0) {
		var_1_1 = (var_1_17 + var_1_17);
	} else {
		var_1_1 = ((min ((var_1_7 + 1) , var_1_8)) - (24762 - var_1_9));
	}


	// From: Req6Batch15PS_CN_500
	signed long int stepLocal_3 = var_1_22 + var_1_11;
	signed long int stepLocal_2 = (var_1_22 - var_1_23) - var_1_9;
	if (var_1_20) {
		if (var_1_10 < stepLocal_2) {
			if (var_1_19) {
				if (last_1_var_1_21 == stepLocal_3) {
					var_1_21 = var_1_1;
				}
			} else {
				var_1_21 = var_1_10;
			}
		} else {
			var_1_21 = var_1_11;
		}
	} else {
		var_1_21 = var_1_9;
	}
}



void updateVariables() {
	var_1_3 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_3 >= -922337.2036854776000e+13F && var_1_3 <= -1.0e-20F) || (var_1_3 <= 9223372.036854776000e+12F && var_1_3 >= 1.0e-20F ));
	var_1_4 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_4 >= -922337.2036854776000e+13F && var_1_4 <= -1.0e-20F) || (var_1_4 <= 9223372.036854776000e+12F && var_1_4 >= 1.0e-20F ));
	var_1_7 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_7 >= 0);
	assume_abort_if_not(var_1_7 <= 16383);
	var_1_8 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_8 >= -1);
	assume_abort_if_not(var_1_8 <= 32766);
	var_1_9 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_9 >= 0);
	assume_abort_if_not(var_1_9 <= 16383);
	var_1_11 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_11 >= -127);
	assume_abort_if_not(var_1_11 <= 126);
	var_1_13 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_13 >= -922337.2036854776000e+13F && var_1_13 <= -1.0e-20F) || (var_1_13 <= 9223372.036854776000e+12F && var_1_13 >= 1.0e-20F ));
	var_1_14 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_14 >= -922337.2036854776000e+13F && var_1_14 <= -1.0e-20F) || (var_1_14 <= 9223372.036854776000e+12F && var_1_14 >= 1.0e-20F ));
	var_1_15 = __VERIFIER_nondet_double();
	assume_abort_if_not((var_1_15 >= 0.0F && var_1_15 <= -1.0e-20F) || (var_1_15 <= 9223372.036854765600e+12F && var_1_15 >= 1.0e-20F ));
	var_1_16 = __VERIFIER_nondet_double();
	assume_abort_if_not((var_1_16 >= 0.0F && var_1_16 <= -1.0e-20F) || (var_1_16 <= 9223372.036854765600e+12F && var_1_16 >= 1.0e-20F ));
	var_1_20 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_20 >= 0);
	assume_abort_if_not(var_1_20 <= 0);
	var_1_22 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_22 >= 16383);
	assume_abort_if_not(var_1_22 <= 32767);
	var_1_23 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_23 >= 0);
	assume_abort_if_not(var_1_23 <= 16384);
	var_1_27 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_27 >= 0);
	assume_abort_if_not(var_1_27 <= 1);
	var_1_28 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_28 >= -32766);
	assume_abort_if_not(var_1_28 <= 32766);
	var_1_29 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_29 >= -32766);
	assume_abort_if_not(var_1_29 <= 32766);
	var_1_30 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_30 >= 0);
	assume_abort_if_not(var_1_30 <= 1);
	var_1_31 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_31 >= 0);
	assume_abort_if_not(var_1_31 <= 1);
	var_1_32 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_32 >= -1);
	assume_abort_if_not(var_1_32 <= 32766);
	var_1_33 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_33 >= 0);
	assume_abort_if_not(var_1_33 <= 32766);
	var_1_35 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_35 >= -126);
	assume_abort_if_not(var_1_35 <= 126);
	var_1_36 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_36 >= -63);
	assume_abort_if_not(var_1_36 <= 63);
	var_1_37 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_37 >= -63);
	assume_abort_if_not(var_1_37 <= 63);
	var_1_39 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_39 >= -922337.2036854765600e+13F && var_1_39 <= -1.0e-20F) || (var_1_39 <= 9223372.036854765600e+12F && var_1_39 >= 1.0e-20F ));
	var_1_40 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_40 >= -922337.2036854765600e+13F && var_1_40 <= -1.0e-20F) || (var_1_40 <= 9223372.036854765600e+12F && var_1_40 >= 1.0e-20F ));
	var_1_41 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_41 >= 0.0F && var_1_41 <= -1.0e-20F) || (var_1_41 <= 4611686.018427382800e+12F && var_1_41 >= 1.0e-20F ));
	var_1_42 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_42 >= 0.0F && var_1_42 <= -1.0e-20F) || (var_1_42 <= 2305843.009213691390e+12F && var_1_42 >= 1.0e-20F ));
	var_1_43 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_43 >= 0.0F && var_1_43 <= -1.0e-20F) || (var_1_43 <= 2305843.009213691390e+12F && var_1_43 >= 1.0e-20F ));
	var_1_44 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_44 >= 4611686.018427382800e+12F && var_1_44 <= -1.0e-20F) || (var_1_44 <= 9223372.036854765600e+12F && var_1_44 >= 1.0e-20F ));
	var_1_45 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_45 >= 0.0F && var_1_45 <= -1.0e-20F) || (var_1_45 <= 4611686.018427382800e+12F && var_1_45 >= 1.0e-20F ));
	var_1_48 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_48 >= 0);
	assume_abort_if_not(var_1_48 <= 0);
	var_1_49 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_49 >= 1);
	assume_abort_if_not(var_1_49 <= 1);
	var_1_51 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_51 >= 4611686.018427387900e+12F && var_1_51 <= -1.0e-20F) || (var_1_51 <= 9223372.036854776000e+12F && var_1_51 >= 1.0e-20F ));
	var_1_53 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_53 >= 0);
	assume_abort_if_not(var_1_53 <= 32766);
	var_1_54 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_54 >= -32767);
	assume_abort_if_not(var_1_54 <= 32766);
	var_1_56 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_56 >= 1);
	assume_abort_if_not(var_1_56 <= 1);
	var_1_57 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_57 >= 1);
	assume_abort_if_not(var_1_57 <= 1);
	var_1_59 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_59 >= 127);
	assume_abort_if_not(var_1_59 <= 255);
	var_1_60 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_60 >= 0);
	assume_abort_if_not(var_1_60 <= 127);
	var_1_62 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_62 >= -922337.2036854776000e+13F && var_1_62 <= -1.0e-20F) || (var_1_62 <= 9223372.036854776000e+12F && var_1_62 >= 1.0e-20F ));
	var_1_64 = __VERIFIER_nondet_long();
	assume_abort_if_not(var_1_64 >= 1073741823);
	assume_abort_if_not(var_1_64 <= 2147483647);
	var_1_71 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_71 >= 8191);
	assume_abort_if_not(var_1_71 <= 16383);
	var_1_73 = __VERIFIER_nondet_short();
	assume_abort_if_not(var_1_73 >= -32767);
	assume_abort_if_not(var_1_73 <= 32767);
	var_1_77 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_77 >= 0);
	assume_abort_if_not(var_1_77 <= 65535);
	assume_abort_if_not(var_1_77 != 0);
	var_1_79 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_79 >= -128);
	assume_abort_if_not(var_1_79 <= 127);
	assume_abort_if_not(var_1_79 != 0);
	var_1_83 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_83 >= 62);
	assume_abort_if_not(var_1_83 <= 126);
	var_1_84 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_84 >= 0);
	assume_abort_if_not(var_1_84 <= 63);
	var_1_85 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_85 >= 0);
	assume_abort_if_not(var_1_85 <= 126);
	var_1_86 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_86 >= 0);
	assume_abort_if_not(var_1_86 <= 126);
	var_1_88 = __VERIFIER_nondet_ulong();
	assume_abort_if_not(var_1_88 >= 1073741823);
	assume_abort_if_not(var_1_88 <= 2147483647);
	var_1_90 = __VERIFIER_nondet_uchar();
	assume_abort_if_not(var_1_90 >= 0);
	assume_abort_if_not(var_1_90 <= 0);
	var_1_94 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_94 >= 32767);
	assume_abort_if_not(var_1_94 <= 65534);
	var_1_96 = __VERIFIER_nondet_char();
	assume_abort_if_not(var_1_96 >= -1);
	assume_abort_if_not(var_1_96 <= 127);
	var_1_98 = __VERIFIER_nondet_float();
	assume_abort_if_not((var_1_98 >= -922337.2036854776000e+13F && var_1_98 <= -1.0e-20F) || (var_1_98 <= 9223372.036854776000e+12F && var_1_98 >= 1.0e-20F ));
	assume_abort_if_not(var_1_98 != 0.0F);
	var_1_106 = __VERIFIER_nondet_ulong();
	assume_abort_if_not(var_1_106 >= 1610612735);
	assume_abort_if_not(var_1_106 <= 2147483647);
	var_1_112 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_112 >= 0);
	assume_abort_if_not(var_1_112 <= 65535);
	var_1_114 = __VERIFIER_nondet_ulong();
	assume_abort_if_not(var_1_114 >= 2147483647);
	assume_abort_if_not(var_1_114 <= 4294967294);
	var_1_129 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_129 >= 24575);
	assume_abort_if_not(var_1_129 <= 32767);
	var_1_130 = __VERIFIER_nondet_ushort();
	assume_abort_if_not(var_1_130 >= 16384);
	assume_abort_if_not(var_1_130 <= 32767);
}



void updateLastVariables() {
	last_1_var_1_10 = var_1_10;
	last_1_var_1_21 = var_1_21;
	last_1_var_1_25 = var_1_25;
}

int property() {
	return (((((((var_1_19 || (var_1_12 > var_1_12)) ? (var_1_1 == ((signed short int) (var_1_17 + var_1_17))) : (var_1_1 == ((signed short int) ((min ((var_1_7 + 1) , var_1_8)) - (24762 - var_1_9))))) && (var_1_10 == ((signed char) var_1_11))) && ((var_1_4 >= (var_1_3 * (var_1_13 + var_1_14))) ? (var_1_12 == ((double) (min ((var_1_15 - var_1_16) , -0.85)))) : (var_1_12 == ((double) var_1_16)))) && ((var_1_12 <= (min (var_1_12 , var_1_12))) ? ((var_1_19 || var_1_19) ? (var_1_17 == ((signed char) var_1_11)) : (var_1_17 == ((signed char) var_1_11))) : 1)) && ((last_1_var_1_10 < last_1_var_1_25) ? ((var_1_11 <= last_1_var_1_25) ? (var_1_19 == ((unsigned char) var_1_20)) : 1) : 1)) && (var_1_20 ? ((var_1_10 < ((var_1_22 - var_1_23) - var_1_9)) ? (var_1_19 ? ((last_1_var_1_21 == (var_1_22 + var_1_11)) ? (var_1_21 == ((signed long int) var_1_1)) : 1) : (var_1_21 == ((signed long int) var_1_10))) : (var_1_21 == ((signed long int) var_1_11))) : (var_1_21 == ((signed long int) var_1_9)))) && (var_1_19 ? (var_1_25 == ((signed long int) (var_1_17 + (min (var_1_17 , (min (var_1_8 , var_1_17))))))) : (var_1_25 == ((signed long int) var_1_10)))
;
}
int main(void) {
	isInitial = 1;
	initially();

	while (1) {
		updateLastVariables();

		updateVariables();
		step();
		__VERIFIER_assert(property());
		isInitial = 0;
	}

	return 0;
}
